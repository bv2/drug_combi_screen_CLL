...
Problematic are: Afatinib - P0050, Idelalisib - P0050 as no good fit for Ibrutinib or Idelalisib response curve (flat).
Use a criterion on goodness of fit for single curves to include patient? When not limiting the maximal effect of loglogistic fit to 100, more cases become problematic, e.g. P0016 for Navitoclax.
...
if(class(single.fit$drug.col.model) != "drc" | class(single.fit$drug.row.model)!= "drc") 
  stop("Drug response fits did not converge")
...
ggLoeweCI <- ggplot(df4plot,aes(x=factor(round(concBvalue*1000,1)), y = factor(round(concCvalue*1000,1)), fill = log(CILoewe)))  + geom_tile() +
...
```{r}
ggLoewe_effect <- ggplot(df4plot, aes(x=factor(round(concBvalue*1000,1)), y = factor(round(concCvalue*1000,1)), fill = effect - effect_Loewe))   + geom_tile() +
  ylab("Ibrutinib (nM)") + xlab(paste(dr, "(nM)")) +
...
Values above 0 indicate synergy.
```{r}
ggLoewe_effect_nearest <- ggplot(df4plot, aes(x=factor(round(concBvalue*1000,1)),
                    y = factor(round(concCvalue*1000,1)),
                    fill = effect - effect_Loewe_nearest))  +
  geom_tile() +
  ylab("Ibrutinib (nM)") + xlab(paste(dr, "(nM)")) +
          scale_fill_gradient2(low= "blue", high="red", mid="white", midpoint = 0) +
           coord_fixed() +
          theme(axis.text.x = element_text(angle=90, vjust=1, hjust=1))
ggLoewe_effect_nearest
```
...
ggZIP <- ggplot(df_score, aes(x = factor(round(concB*1000,1)),
                              y=factor(round(concC*1000,1)),
                              fill = score)) +
          geom_tile() +
          ylab("Ibrutinib (nM)") + xlab(paste(dr, "(nM)")) +
          scale_fill_gradient2(low= "blue", high="red", mid="white", midpoint = 0) +
...
cowplot::plot_grid(ggZIP +ggtitle("ZIP"),
                   ggLoewe_effect_nearest + ggtitle("Loewe_effect_nearest"),
                   ggLoewe_effect+ ggtitle("Loewe_effect"),
                   ggLoeweCI + ggtitle("Loewe_CI"),
                   ncol=2)
```
