...
### Overall
```{r Ibru100vsIdel, fig.height=9, fig.width=7.5}
...
df4ana %>% filter(BDrugName %in% c("Afatinib","Spebrutinib"), CDrugAbrv %in% c("Ibrutinib (100nM)", "Idelalisib")) %>% 
  ggplot(aes(x=BDrugConcId, y=effectBC)) + geom_boxplot(aes(fill=CDrugAbrv)) + facet_wrap(~BDrugName) +
  ggpubr::stat_compare_means(method = "t.test", aes(group=CDrugAbrv, label =  ..p.signif..), hide.ns = TRUE) +
  guides(fill = guide_legend(title="Combination compound")) + xlab("concentration") +ylab("viability") +theme_bw()
...
mat <- plotHeatmap(df4ana = df4ana, dfMuts = dfMuts4testing, CDrugAbrv4plot = "Ibrutinib (100nM)", type = "PatDrug",
                   useAverage = FALSE, returnMat = TRUE)
```
...
plotHeatmap(df4ana = df4ana, dfMuts = dfMuts4testing, CDrugAbrv4plot = "Idelalisib", type = "PatDrug", useAverage = FALSE)
```

Average over concentrations
```{r Combi_Idea_heatmap_patient_drug_av}
plotHeatmap(df4ana = df4ana, dfMuts = dfMuts4testing, CDrugAbrv4plot = "Idelalisib", type = "PatDrug", useAverage = TRUE)
```
...
ggplot(df4plot, aes(x=mean.diff, y=-log10(pval.adj), col=col)) +
  ...
  ggrepel::geom_label_repel(aes(label=ifelse(sig, BDrugName, "")), label.size= 0, min.segment.length = 0, fill = NA) +
...
```{r boxplot_combiIbru100_TP53, fig.width=6, fig.height=3.5}
