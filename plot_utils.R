...
plotComparsionCDrugs <- function(CDrugAbrv.x, CDrugAbrv.y, range = c(0,1.4), type=c("scatter_joint", "scatter_factBDrug", "boxplot_joint")){
...
plotHeatmap <- function(df4ana, dfMuts, CDrugAbrv4plot, type=c("PatPat", "PatDrug"), useAverage=FALSE,
                        conc4average = paste0("c",1:5), nOcc = 10){
  ...
  if(type=="PatPat"){
    ...
    if(!useAverage){
      ...
      pheatmap(effectBPlusC_mat, na_col="gray", clustering_distance_rows="correlation",
               ...
               color = colorRampPalette(rev(brewer.pal(n = 7, name ="RdYlBu")))(140),
               breaks=seq(0,filter_th,0.01), show_rownames = FALSE, show_colnames = TRUE,
               treeheight_row = 15, treeheight_col = 15, annotation_row = dfanno, annotation_colors =anno_colors, annotation_legend = FALSE)
      }
