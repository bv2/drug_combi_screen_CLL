...
plotBaseBoxplots(df_effectB_vs_muts, df_signTP53$BDrugName, gene = "TP53")
```
```{r curves_base_TP53, fig.width=7, fig.height=2.8}
plotBaseResponseCurves(df_effectB_vs_muts, df_signTP53$BDrugName, gene = "TP53")
```

```{r, boxplot_base_IGHV, fig.width=8, fig.height=5}
selDrugs <- c("Ibrutinib", "R406", "Spebrutinib", "AZD7762", "PF 477736")
stopifnot(all(selDrugs %in% df_signIGHV$BDrugName))
plotBaseBoxplots(df_effectB_vs_muts, selDrugs, gene = "IGHV")
```

```{r curves_base_IGHV, fig.width=7, fig.height =5}
...
names(anno_colors) <- colnames(dfanno)
 ...
 pheatmap(effectB_mat_av, na_col="gray", clustering_distance_rows="euclidean",
         ...
         annotation_legend = FALSE, fontsize = 12)
```
