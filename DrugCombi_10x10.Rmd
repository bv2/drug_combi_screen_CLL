...
for(dr in unique(df10x10$BaseDrugName)){
  if(dr!="DM"){
    print(dr)
...
print(gg)
print(round(mean(df_score$score),3))
...
drugCombo::checkerboardData
 data <- filter(df10x10, BaseDrugName == "Afatinib", CombiDrug == "Ibrutinib") %>%
  select(exp = PatientID, d1 = concBvalue, d2 = concCvalue, effect = normalizedValue)
# fit <- drugCombo::fitModel(data, model = "separate12")
fit <- drugCombo::fitModel(data, mono =NULL, model = "separate12")
summary(fit)
plot(fit)
plot(fit, which = "3d")
