...
```
Correct concentrations according (wrong in tables):
2µM for navitoclax and venetoclax and 20µM for afatinib, idelalisib and ibrutinib in 2-fold serial dilutions, ending at 0
```{r}
...
df10x10 %<>% mutate(concBvalue = ifelse(BaseDrugName %in% c("Afatinib", "Idelalisib"), conc_other[base_conc],
                                 ifelse(BaseDrugName %in% c("Navitoclax", "Venetoclax"),  conc_BCL2[base_conc], NA)))
df10x10 %<>% mutate(concCvalue = ifelse(CombiDrug == "Ibrutinib", conc_other[combi_conc], NA))
```
...
gg <- ggplot(df_score, aes(x = factor(concB), y=factor(concC), fill = score)) +
  geom_tile() +
  ylab("Ibrutinib (μM)") + xlab(paste(dr, "(μM)")) +
