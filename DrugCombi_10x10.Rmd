...
plotCITiles(df10x10, "Loewe")
```

```{r ZIP_surface}
plotCITiles(df10x10, "ZIP")
```
...
load("out/MarinaCombiSyns_180905.RData")
...
df4plot %>% ggplot(aes(x=pat, y=score, fill=pat)) + 
  facet_grid(method~drB, scales = "free") + geom_bar(stat="identity") + theme(axis.text.x = element_blank()) +guides(fill =F) +scale_fill_manual(values=patcol) +geom_hline(yintercept = 0)
```


Other packages: drc, SYNERGY
```{r, eval = FALSE}
# fit a durface across all patients with variable tau (RHS in the Loewe additivity model, no synergy =1)
# only work for synergistic combinations, not practical.
library(drugCombo)
drugCombo::checkerboardData
df <- lapply(c("Afatinib", "Navitoclax", "Venetoclax", "Idelalisib"), function(drB){
 data <- filter(df10x10, BaseDrugName == drB, CombiDrug == "Ibrutinib") %>%
  select(exp = PatientID, d1 = concBvalue, d2 = concCvalue, effect = normalizedValue) 
 print(summary(data$effect))
# fit <- drugCombo::fitModel(data, model = "separate12")
fit <- drugCombo::fitModel(data, mono =NULL, model = "uniform")
taus <- drugCombo::getTauSurface(fit)
data.frame(taus$data[1,3:6], drB = drB)
}) %>% bind_rows()
df
```
