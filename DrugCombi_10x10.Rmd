...
for(dr in unique(df10x10$BaseDrug)){
  df <- filter(df10x10, BaseDrugName == dr)
  for(pat in unique(df$PatientID)){
      df <- filter(df10x10, PatientID == pat) %>%
...
data$dose.response.mats[[1]] <- data$dose.response.mats[[1]][order(rownames(data$dose.response.mats[[1]])), order(colnames(data$dose.response.mats[[1]]))]
PlotDoseResponse(data)

synergy.score <- CalculateSynergy(data,method = "Loewe", correction = TRUE, Emin = 0, Emax = 100)
PlotSynergy(synergy.score, type = "2D")
  }
  }
```
