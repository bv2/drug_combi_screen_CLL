...
ggNavi <- plotSummaryLoewe("Navitoclax", type = "row")
ggNavi
```

```{r summary_Afatinib, fig.width = 10 }
ggAfa <- plotSummaryLoewe("Afatinib", type ="row")
ggAfa
```

```{r summary_Venetoclax, fig.width = 10 }
ggVen <- plotSummaryLoewe("Venetoclax", type ="row")
ggVen
```

```{r summary_Idelalisib, fig.height = 8 }
ggIdel <- plotSummaryLoewe("Idelalisib")
ggIdel
```
