...
# no major changes:
# dfLoewe <- plotCITiles(filter( df10x10, BaseDrugName != "Idelalisib"), "Loewe", cutoff = 100)
# dfLoewe_nocut <- plotCITiles(filter( df10x10, BaseDrugName == "Idelalisib"), "Loewe")
```
...
```{r summary_Navitoclax, fig.width = 10 }
ggNavi <- plotSummaryLoewe(dfLoewe = dfLoewe, "Navitoclax", type = "row")
ggNavi
```

```{r summary_Afatinib, fig.width = 10 }
ggAfa <- plotSummaryLoewe(dfLoewe = dfLoewe, "Afatinib", type ="row")
ggAfa
```

```{r summary_Venetoclax, fig.width = 10 }
ggVen <- plotSummaryLoewe(dfLoewe = dfLoewe, "Venetoclax", type ="row")
ggVen
```

```{r summary_Idelalisib, fig.height = 8 }
ggIdel <- plotSummaryLoewe(dfLoewe = dfLoewe_nocut, "Idelalisib")
ggIdel
```

```{r summary_joint, fig.width=10, fig.height=12}
cowplot::plot_grid(ggNavi, ggAfa, ggVen, ggIdel)
```
