...
pheatmap(SImat, breaks = seq(0, max(SImat),0.01), main = "SI")
...
pheatmap(SImat, breaks = seq(0,1,0.01), main = "Highest single agent Index")
```
...
pheatmap(SImat, breaks = seq(0,1,0.01), main = "Bliss Index")
```
...
myHeatmap(-log10(pmat), colors=c("white", "white", "white", "grey","black"))
...
```{r Ibrutinib (100nM)_Navitoclax_Curves, fig.width=6}
plotResponseCurves(df =dfsyn, drC = "Ibrutinib (100nM)", drB = "Navitoclax")
```

## Bxplots CI
```{r Ibrutinib (100nM)_Navitoclax_CIboxplots}
...
```{r Ibrutinib (100nM)_Navitoclax_CIwaterfall, fig.width=7}
...
## Response curves
Viability response curves (mean across patients) for single drugs, combination and additive effect model.
```{r Ibrutinib (100nM)_Afatinib_Curves, fig.width=6}
plotResponseCurves(df =dfsyn, drC = "Ibrutinib (100nM)", drB = "Afatinib", sep_by_IGHV = TRUE)
```

## Boxplots CI
```{r Ibrutinib (100nM)_Afatinib_CIboxplots}
...
```{r Ibrutinib (100nM)_Afatinib_CIwaterfall, fig.width=7}
...
## Response curves
Viability response curves (mean across patients) for single drugs, combination and additive effect model.
```{r Ibrutinib (100nM)_Venetoclax_Curves, fig.width=6}
plotResponseCurves(df =dfsyn, drC = "Ibrutinib (100nM)", drB = "Venetoclax", sep_by_IGHV = TRUE)
```

## Boxplots CI
```{r Ibrutinib (100nM)_Venetoclax_CIboxplots}
...
```{r Ibrutinib (100nM)_Venetoclax_CIwaterfall, fig.width=7}
...
## Response curves
Viability response curves (mean across patients) for single drugs, combination and additive effect model.
```{r Ibrutinib (100nM)_Fludarabine_Curves, fig.width=6}
plotResponseCurves(df =dfsyn, drC = "Ibrutinib (100nM)", drB = "Fludarabine", sep_by_IGHV = TRUE)
```

## Boxplots CI
```{r Ibrutinib (100nM)_Fludarabine_CIboxplots}
...
```{r Ibrutinib (100nM)_Fludarabine_CIwaterfall, fig.width=7}
...
dfsynSummaryConcPat %>% filter(CDrugAbrv %in% BCRi) %>%
  ...
  facet_wrap(~CDrugAbrv, ncol=1) +guides(fill=FALSE) + theme(axis.text.x = element_text(angle=60, vjust=1, hjust=1))  +
  ylab("Synergy Index (SI)") + xlab("Base compound")
```

## Response curves
```{r BCRi_ResponseCurves}
