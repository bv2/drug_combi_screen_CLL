...
ggplot(df4hm, aes(x=CDrugAbrv, y= BDrugName, fill =addModelSImed, label = ifelse(pvaladj <= alpha, "*", ""))) +
  ...
  ylab("Base library") + xlab("Compination compound") +
...
# df_pvals_SI_vs_muts$pval.adj <-  ihw(df_pvals_SI_vs_muts$pval, factor(df_pvals_SI_vs_muts$CDrugAbrv), alpha = 0.05)
# ggplot(df_pvals_SI_vs_muts, aes(x=pval, fill= CDrugAbrv)) +geom_histogram() +facet_wrap(~CDrugAbrv)
...
plotWaterfallCI(dfsynSummaryConc, drC = "Ibrutinib (100nM)", drB = "Navitoclax", CI_type = "SI") #+theme(axis.text.x = element_text(angle=90))
```
...
plotWaterfallCI(dfsynSummaryConc, drC = "Ibrutinib (100nM)", drB = "Venetoclax", CI_type = "SI") +theme(axis.text.x = element_text(angle=90))
```
...
## Waterfall plot SI
```{r BCRi_WaterfallSI, fig.height= 7, fig.width=9}
...
## Response curves
```{r BCRi_ResponseCurves, fig.height= 8, fig.width=12}
