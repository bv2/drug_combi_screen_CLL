...
# Overview
```{r}
dfsynSummaryConcPat %>% group_by(BDrugName) %>%
  summarize(nSI = sum(addModelSImed>0),
            nHSA = sum(hsaCImed>0)) %>%
  arrange(-nHSA)
```
...
Annotate heatmaps with signifiance by a paired t-test across all patients samples (separately for each concentration)
...
dfsigSI_summary <- filter(dfsigSI, pvaladj<0.05)%>%
...
df4hm <- left_join(select(dfsynSummaryConcPat,CDrugAbrv,BDrugName, addModelSImed),
          dfsigSI_summary, by = c("BDrugName","CDrugAbrv"))

# cases with average SI <0 but signifcant conc with > 0?
stopifnot(nrow(filter(df4hm, !is.na(nConc), addModelSImed<0)) ==0)
...
ggplot(df4hm, aes(x=CDrugAbrv, y= BDrugName, fill =addModelSImed, label = ifelse(!is.na(nConc), "*", ""))) +
...
df4hm <- left_join(select(dfsynSummaryConcPat,CDrugAbrv,BDrugName, hsaCImed),
          dfsigHSA_summary, by = c("BDrugName","CDrugAbrv"))
...
# cases with average HSA <0 but signifcant conc with > 0
# stopifnot(nrow(filter(df4hm, !is.na(nConc), hsaCImed<0)) ==0 )
# plotResponseCurves(dfsyn, "Ibrutinib (100nM)", "Silmitasertib")
# filter(dfsyn,CDrugAbrv=="Ibrutinib (100nM)", BDrugName=="Silmitasertib" , BDrugConcId =="c1")

ggplot(df4hm, aes(x=CDrugAbrv, y= BDrugName, fill =hsaCImed, label = ifelse(!is.na(nConc), "*", ""))) +
  geom_tile() + geom_text(nudge_y=-0.2) +
  scale_fill_gradient2(low="navy", high="darkred", mid = "white", midpoint = 0) +
  theme_bw(base_size = 15) +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
