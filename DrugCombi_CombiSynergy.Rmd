...
# maximum/minimum across concentrations
dfsynSummaryConcPatMax <- dfsynSummaryPat %>%
  group_by(CDrugAbrv,BDrugName) %>%
  summarize(SImax = max(addModelSImed), HSAmax = max(hsaCImed),
            Simin = min(addModelSImed), HSAmin = min(hsaCImed))
...
ggplot(df4hm, aes(x=CDrugAbrv, y= BDrugName, fill =addModelSImed, label = ifelse(pvaladj <= alpha, "*", ""))) +
...
df_pvals_SI_vs_muts <- group_by(dfSI_gen_test, CDrugAbrv, BDrugName,BDrugConcId, BDrugConc, mutation, combi) %>%
...
```{r}
# IGHV assoc.
explSIgen <- function(drB, drC) {
left_join(dfsyn,dfMuts4testing, by = "PatientID") %>%
  filter(BDrugName == drB, CDrugAbrv == drC) %>%
  mutate(IGHV = ifelse(IGHV == 0, "U-CLL", "M-CLL")) %>%
ggplot(aes(x=IGHV, y=addModelSI, group=IGHV, fill= IGHV)) +geom_boxplot() + facet_wrap(~BDrugConc, nrow=1) + 
  ggtitle(paste(drC, drB, sep="+")) + ylab("SI") + ggpubr::stat_compare_means(method = "t.test")
}

gg1 <- explSIgen("Fludarabine","Ibrutinib (100nM)")
gg2 <- explSIgen("Fludarabine","Ibrutinib (10nM)")
gg3 <- explSIgen("Afatinib","Ibrutinib (100nM)")
gg4 <- explSIgen("Afatinib","Ibrutinib (10nM)")
...
left_join(dfsyn,dfMuts4testing, by = "PatientID") %>%
  filter(BDrugName == drB, CDrugAbrv == drC) %>%
  mutate(TP53 = ifelse(TP53 == 0, "TP53 wt", "TP53 mut")) %>%
ggplot(aes(x=TP53, y=addModelSI, group=TP53, fill= TP53)) +geom_boxplot() + facet_wrap(~BDrugConc, nrow=1) + 
  ggtitle(paste(drC, drB, sep="+")) + ylab("SI") + ggpubr::stat_compare_means(method = "t.test")
}
...
```{r}
dfsynSummaryConc %>% filter(CDrugAbrv =="Ibrutinib (100nM)", BDrugName =="Navitoclax") %>%
