...
| ------------- | ------------- |
| “PatientOverview.pdf" | matrix giving an overview over the patients including infromation about sex, age, somatic mutations, IGHV status, which screens they are in and their color in all other plots |
...
##Patient overview including genetic background
Creates a plot called "PatientOverview.pdf" giving an overview over the patients including infromation about sex, age, somatic mutations, IGHV status, which screens they are in and their color in all other plots.
```{r}
#turn feautre matrices for molecular data in dataframe
MolMatrix2DF<-function(M, bin=T){
  df<-data.frame(Patient=rep(rownames(M), times=ncol(M)),
             Feature=rep(colnames(M), each=nrow(M)),
             Value= as.vector(M))
  df$Patient<-as.character(df$Patient)
  if(bin==T){
  df$Value[which(is.na(df$Value))]<-"NA"
...
#Mutations
NoOcc<-colSums(genData, na.rm = T)
df4plotGen<-MolMatrix2DF(genData)
df4plotGen$Feature<-factor(df4plotGen$Feature, levels=colnames(genData)[order(NoOcc)])
ggMut<-ggplot(data=df4plotGen, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_manual(values=c("0"="white","1"="black", "NA"="grey90"), name="Mutation", labels=c("WT","MUT","NA")) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotGen$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotGen$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(),  axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

pdf(file.path(outputdir,"BarplotMutations.pdf"))
dfGen<-MolMatrix2DF(genData, bin=F)
dfMutOcc<-dplyr::group_by(dfGen,Feature) %>% dplyr::summarise(Nocc=mean(sum((Value), na.rm=T)))
dfMutOcc$Feature<-factor(dfMutOcc$Feature, levels=dfMutOcc$Feature[order(dfMutOcc$Nocc, decreasing = T)])
ggplot(filter(dfMutOcc, Nocc>2), aes(x=Feature, y=Nocc)) +geom_bar(stat="identity", fill="darkred") +ylab("Number of samples")+
   theme(axis.text.x = element_text(angle=90, hjust = 1))+ggtitle("Genetic alterations")+xlab("")
dev.off()

#IGHV
df4plotIGHV<-MolMatrix2DF(IGHVData)
ggIGHV<-ggplot(data=df4plotIGHV, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_manual(values=c("0"="white","1"="darkgreen", "NA"="grey90"), name="IGHV", labels=c("U-CLL","M-CLL","NA")) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotIGHV$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotIGHV$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(), axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

#sex
df4plotSex<-MolMatrix2DF(clinicalData[,"gender", drop=F])
ggGender<-ggplot(data=df4plotSex, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_manual(values=c("0"="blue","1"="red", "NA"="grey90"), name="Sex", labels=c("m","w","NA")) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotSex$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotSex$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(),  axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

#age
df4plotAge<-MolMatrix2DF(clinicalData[,"age", drop=F], bin=F)
ggAge<-ggplot(data=df4plotAge, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_gradient(low = "orange", high = "brown", na.value="grey90", name="Age", breaks=c(40,60,80)) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotIGHV$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotIGHV$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(), axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

#pretreatment
df4plotTreatment<-MolMatrix2DF(clinicalData[,"pretreated", drop=F])
ggTreatment<-ggplot(data=df4plotTreatment, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_manual(values=c("0"="white","1"="violetred4", "NA"="grey90"), name="pretreated", labels=c("no","yes","NA")) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotTreatment$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotTreatment$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(),  axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

#screen info
ScreenMat<-matrix(0, ncol=length(unique(df4ana$ScreenNo)), nrow=length(unique(df4ana$PatientID)))
colnames(ScreenMat)<-unique(df4ana$ScreenNo)
rownames(ScreenMat)<-unique(df4ana$PatientID)
for(screen in as.character(unique(df4ana$ScreenNo))){
 patINscreen<- as.character(unique(filter(df4ana, ScreenNo==screen)$PatientID))
 ScreenMat[patINscreen,screen]<-1
}
df4plotScreen<-MolMatrix2DF(ScreenMat)
df4plotScreen$Feature<-factor(df4plotScreen$Feature, levels=paste("SS", 13:1, sep=""))
ggScreens<-ggplot(data=df4plotScreen, aes(x=Patient, y=Feature, fill=Value)) + geom_tile() + scale_fill_manual(values=c("1" = "navy", "0"="white"), name="in screen",labels=c("yes","no")) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotScreen$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotScreen$Feature))+1,1), colour="grey60") + theme(axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) + theme(axis.ticks=element_blank(),  axis.text=element_text(margin=unit(0.5,"cm")), legend.key = element_rect(colour = "black"), legend.text=element_text(size=16), legend.title=element_text(size=16))

#color annotation
patcol_ord<-patcol[order(names(patcol))]
df4plotcolor<-data.frame(Patient=names(patcol_ord), 
                            Feature=rep("color", length(patcol_ord)),
                            Value=names(patcol_ord))
ggColor<-ggplot(data=df4plotcolor, aes(x=Patient, y=Feature, fill=as.character(Value))) + geom_tile() + scale_fill_manual(values=patcol_ord, labels=names(patcol_ord)) + ylab("") + xlab("") + geom_vline(xintercept=seq(0.5,length(unique(df4plotcolor$Patient))+1,1), colour="grey60") + geom_hline(yintercept=seq(0.5,length(unique(df4plotcolor$Feature))+1,1), colour="grey60") + theme(axis.ticks=element_blank(), axis.text.x = element_text(angle=90, hjust=0, vjust=0.5), axis.text = element_text(colour="black")) + scale_y_discrete(expand=c(0,0)) + scale_x_discrete(expand=c(0,0)) 

#put all together
grobMut<-ggplotGrob(ggMut)
grobAge<-ggplotGrob(ggAge)
grobTreatment<-ggplotGrob(ggTreatment)
grobGender<-ggplotGrob(ggGender)
grobScreens<-ggplotGrob(ggScreens)
grobIGHV<-ggplotGrob(ggIGHV)
grobColor<-ggplotGrob(ggColor)
heights4Fig<-c(1,1,1,0.2,1,0.2,ncol(ScreenMat),0.2, ncol(genData),0.2,1,0.2,1)/5
widths4Fig<-c(5,25,5)/3
gt<-gtable(width<-unit(widths4Fig, "in"), heights = unit(heights4Fig,"in"))
gt = gtable_add_grob(gt, grobGender$grobs[[4]], 1, 2)
gt = gtable_add_grob(gt, grobAge$grobs[[4]], 2, 2)
gt = gtable_add_grob(gt, grobTreatment$grobs[[4]], 3, 2)
gt = gtable_add_grob(gt, grobIGHV$grobs[[4]], 5, 2)
gt = gtable_add_grob(gt, grobScreens$grobs[[4]], 7, 2)
gt = gtable_add_grob(gt, grobMut$grobs[[4]], 9, 2)
gt = gtable_add_grob(gt, grobColor$grobs[[4]], 11, 2)
# add x axis
gt = gtable_add_grob(gt, grobMut$grobs[[5]], 13, 2)

# add y axis
gt = gtable_add_grob(gt, grobGender$grobs[[2]], 1, 1)
gt = gtable_add_grob(gt, grobAge$grobs[[2]], 2, 1)
gt = gtable_add_grob(gt, grobTreatment$grobs[[2]], 3, 1)

gt = gtable_add_grob(gt, grobIGHV$grobs[[2]], 5,1)
gt = gtable_add_grob(gt, grobScreens$grobs[[2]], 7, 1)
gt = gtable_add_grob(gt, grobMut$grobs[[2]], 9, 1)
gt = gtable_add_grob(gt, grobColor$grobs[[2]], 11, 1)

#add legend
gtlegend= gtable(width<-unit(widths4Fig, "in"), heights = unit(c(5,rep(c(2,0.2), times=6),5),"in"))
gtlegend= gtable_add_grob(gtlegend, grobGender$grobs[[8]],2,3)
gtlegend= gtable_add_grob(gtlegend, grobAge$grobs[[8]],4,3)
gtlegend= gtable_add_grob(gtlegend, grobTreatment$grobs[[8]],6,3)
gtlegend= gtable_add_grob(gtlegend, grobIGHV$grobs[[8]],8,3)
gtlegend= gtable_add_grob(gtlegend, grobScreens$grobs[[8]],10,3)
gtlegend= gtable_add_grob(gtlegend, grobMut$grobs[[8]],12,3)


 pdf(file.path(outputdir,"PatientOverview.pdf"), width=sum(widths4Fig)*1.2, height=sum(heights4Fig)*1.2)
grid.draw(gt)
grid.draw(gtlegend)
dev.off()
```
